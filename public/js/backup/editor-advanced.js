 ...
 * - Third-Person Perspective (Explore Mode) - like House Flipper + Infinitown open world
 * - Non-inverted mouse: mouse right = camera right
 * - Gravity + jump (SPACE) with proper ground collision
 * - Build Mode with Unity-like editor (TransformControls)
 * - Furniture toggle: click active item again to deselect
 * - Procedural open world city in explore mode
 */
...
class AdvancedRoom3DEditorV4 {
    constructor() {
        ...
        this.gravity = -18;        // Gravity for snappy feel
        this.jumpForce = 8;        // Jump velocity
        this.isGrounded = false;
        this.groundLevel = 0;      // Y position of ground (will be set in setupRoom)
        this.characterCollider = null;

        // Open world (Infinitown-style)
        this.openWorldObjects = [];
        this.worldRadius = 60;
    ...
    init() {
        ...
        this.setupUI();
        this.loadRoomData();
        this.generateOpenWorld();   // ← Infinitown-style world
    ...
    setupScene() {
        ...
        this.scene.fog = new THREE.FogExp2(0x87ceeb, 0.012); // exponential fog
    }
    ...
    createRoomGeometry() {
        ...
        // Large seamless ground for open world (below room floor)
        if (!this.groundMesh) {
            const groundGeom = new THREE.PlaneGeometry(500, 500);
            const groundMat = new THREE.MeshStandardMaterial({
                color: 0x5a8a3c,   // grass green
                roughness: 0.9,
                metalness: 0
            });
            ...
            // Set groundLevel to match ground mesh position
            this.groundLevel = 0.1; // Slightly above ground to prevent clipping
        }
    }

    setupCharacter() {
        this.character = new THREE.Group();
        // Set position with proper ground offset
        this.character.position.set(0, 0.2, 2); // Slightly above ground level

        // Fallback character (blocky House-Flipper style)
        const characterGroup = new THREE.Group();

        // Body
        const bodyGeom = new THREE.CylinderGeometry(0.2, 0.25, 0.9, 8);
        ...
        // Legs
        const legGeom = new THREE.CylinderGeometry(0.09, 0.09, 0.7, 8);
        const legMat = new THREE.MeshStandardMaterial({ color: 0x222266 });
        for (let lx of [-0.12, 0.12]) {
    ...
    onMouseMove(e) {
        if (this.mode === 'explore') {
            ...
            // NON-INVERTED: mouse right → camera turns right (positive yaw)
            ...
            // Middle mouse to orbit camera in build mode
            this.cameraYaw += e.movementX * this.cameraRotationSpeed;
            this.cameraPitch -= e.movementY * this.cameraRotationSpeed;
            this.cameraPitch = Math.max(-Math.PI / 2.5, Math.min(Math.PI / 2.5, this.cameraPitch));
        } else if (this.mode === 'build' && this.mouseDown.right) {
            // Right mouse to zoom in build mode
    ...
    handleUIClick(e) {
        ...
        if (target.id === 'toggle-mode-btn') {
            ...
            if (this.currentFurnitureType === type) {
                this.setFurnitureType(null);   // deselect → clicking floor won't place anything
            } else {
    ...
    setFurnitureType(type) {
        ...
        document.querySelectorAll('.furniture-item').forEach(item => {
            item.classList.toggle('active', type !== null && item.dataset.type === type);
        });
    }
    ...
    addFurniture(type, position) {
        ...
        furniture.userData.width = this.getFurnitureDimensions(type).width;
        furniture.userData.height = this.getFurnitureHeight(type);
        furniture.userData.depth = this.getFurnitureDimensions(type).depth;
    ...
    toggleMode() {
        ...
        // Hide character in build mode, show in explore mode
        if (this.character) {
            this.character.visible = (this.mode === 'explore');
        }
    ...
    updateCameraPosition() {
        if (this.mode === 'explore') {
            ...
            if (moveVector.length() > 0) {
                moveVector.normalize().multiplyScalar(this.cameraSpeed * 0.016);
            ...
            // ── TPP Camera ──
            // Camera orbits behind character - proper orientation:
            // yaw=0: behind (+Z), yaw=90°: right (+X), yaw=180°: front (-Z), yaw=270°: left (-X)
            const sinYaw = Math.sin(this.cameraYaw);
            const cosYaw = Math.cos(this.cameraYaw);
            const sinPitch = Math.sin(this.cameraPitch);
            const cosPitch = Math.cos(this.cameraPitch);

            // Position camera behind character with orbit
            const camX = this.character.position.x + sinYaw * this.cameraDistance * cosPitch;
            const camY = this.character.position.y + this.cameraHeight + sinPitch * this.cameraDistance;
            const camZ = this.character.position.z + cosYaw * this.cameraDistance * cosPitch;
            ...
            this.camera.position.set(
                centerPos.x + sinYaw * this.cameraDistance * cosPitch,
                centerPos.y + sinPitch * this.cameraDistance + 2,
                centerPos.z + cosYaw * this.cameraDistance * cosPitch
            );
    ...
    updatePhysics(delta) {
        ...
        // Ground collision - snap to groundLevel (0.1 to keep above ground)
        if (this.character.position.y <= 0.1) {
    ...
    /**
     * Generate Infinitown-style procedural open world city around the room
     */
    generateOpenWorld() {
        if (this.worldGenerated) return;
        ...
        // Palette of building colors (Infinitown-inspired pastel city)
        const buildingColors = [
            0xff6b6b, 0xffa07a, 0xffd700, 0x98fb98,
            0x87ceeb, 0xdda0dd, 0xf0e68c, 0xff8c94,
            0xa8d8ea, 0xffc8a2, 0xb5ead7, 0xc7ceea
        ];
        const roofColors = [0x8b4513, 0x556b2f, 0x8b0000, 0x2f4f4f, 0x6a5acd];

        // Procedural hash for deterministic generation
        const hash = (x, z) => {
            let h = Math.sin(x * 127.1 + z * 311.7 + seed) * 43758.5453;
            return h - Math.floor(h);
        };

        const roomHalfW = this.roomWidth / 2 + 2;
        const roomHalfL = this.roomLength / 2 + 2;

        for (let x = -radius; x < radius; x += blockSize) {
            for (let z = -radius; z < radius; z += blockSize) {
                // Skip cells near the room
                if (Math.abs(x) < roomHalfW && Math.abs(z) < roomHalfL) continue;

                const r = hash(x, z);
                if (r < 0.35) continue; // 35% empty lots

                const bw = 2.5 + hash(x + 1, z) * 2.5;
                const bd = 2.5 + hash(x, z + 1) * 2.5;
                const bh = 2 + hash(x + 2, z + 2) * 10; // 2–12 units tall
                const colorIdx = Math.floor(hash(x + 3, z + 3) * buildingColors.length);
                const roofIdx = Math.floor(hash(x + 4, z) * roofColors.length);
                ...
                const windowMat = new THREE.MeshStandardMaterial({
                    ...
                    metalness: 0.6,
                    roughness: 0.1,
                    emissive: hash(x + 5, z) > 0.5 ? 0xffff99 : 0x000000,
                    emissiveIntensity: 0.3
                });
                for (let wy = wSpacingY; wy < bh - 0.5; wy += wSpacingY) {
                    for (let wx = -bw / 2 + 0.4; wx < bw / 2; wx += wSpacingX) {
                        const w = new THREE.Mesh(new THREE.BoxGeometry(wSize, wSize, 0.05), windowMat);
                        w.position.set(wx, wy, bd / 2 + 0.01);
                        group.add(w);
                    }
                }

                // Roof
                const hasPointRoof = hash(x + 6, z + 6) > 0.6;
                if (hasPointRoof) {
                    const roofH = 1.5 + hash(x, z + 7) * 2;
                    const roofGeom = new THREE.ConeGeometry(Math.max(bw, bd) * 0.65, roofH, 4);
                    const roofMat = new THREE.MeshStandardMaterial({ color: roofColors[roofIdx] });
                    ...
                    const parapetGeom = new THREE.BoxGeometry(bw + 0.2, 0.2, bd + 0.2);
                    const parapet = new THREE.Mesh(parapetGeom, bodyMat);
                    parapet.position.y = bh + 0.1;
                    group.add(parapet);
                }

                group.position.set(
                    x + (hash(x + 8, z) - 0.5) * 1.5,
                    0,
                    z + (hash(x, z + 8) - 0.5) * 1.5
                );
                this.scene.add(group);
                this.openWorldObjects.push(group);
            }
        }

        // Roads
        this.generateRoads(radius);

        // Trees scattered around
        this.generateTrees(radius);

        console.log(`🌆 Open world generated: ${this.openWorldObjects.length} buildings`);
    }

    generateRoads(radius) {
        const roadMat = new THREE.MeshStandardMaterial({ color: 0x333333, roughness: 0.9 });
        const roadWidth = 2.2;
        const spacing = 12;

        for (let x = -radius; x <= radius; x += spacing) {
            const g = new THREE.PlaneGeometry(roadWidth, radius * 2);
            const m = new THREE.Mesh(g, roadMat);
            m.rotation.x = -Math.PI / 2;
            m.position.set(x, 0.005, 0);
            m.receiveShadow = true;
            this.scene.add(m);
            this.openWorldObjects.push(m);
        }

        for (let z = -radius; z <= radius; z += spacing) {
            const g = new THREE.PlaneGeometry(radius * 2, roadWidth);
            const m = new THREE.Mesh(g, roadMat);
            m.rotation.x = -Math.PI / 2;
    ...
    generateTrees(radius) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const trunk = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.3, 2, 7), trunkMat);
            trunk.position.y = 1;
            trunk.castShadow = true;
            group.add(trunk);

            const leafMat = new THREE.MeshStandardMaterial({ color: leafColors[i % leafColors.length] });
            const leaves = new THREE.Mesh(new THREE.SphereGeometry(1.2 + (i % 3) * 0.3, 7, 7), leafMat);
            leaves.position.y = 3;
            ...
            <div class="ui-panel top-left">
                ...
                ${this.mode === 'explore' ? `
                    <div class="help-text">
                        ...
                        Click to lock mouse | E: Build Mode
                    </div>
                ` : `
                    <div class="help-text">
                        <strong>🛠️ Build Mode</strong><br/>
                        Click furniture to toggle placement | Click floor to place<br/>
                        Click again to deselect tool | Q: Move | W: Rotate | R: Scale<br/>
                        MMB drag: Orbit | RMB drag: Zoom | E: Explore
                    </div>
                `}
            </div>
