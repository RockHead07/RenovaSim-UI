            ...
            @else
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($projects as $project)
                        @php
                            $est         = $project->estimations->first();
                            $costMin     = (int) ($est?->cost_min ?? 0);
                            $costMax     = (int) ($est?->cost_max ?? 0);
                            $costAvg     = $costMin > 0 ? (int)(($costMin + $costMax) / 2) : 0;
                            $jobTypeKey  = $est?->job_type;
                            $jobLabel    = $jobTypeKey ? ($jobTypeMap[$jobTypeKey] ?? $jobTypeKey) : null;
                            $confLabel   = $est?->confidence_label;
                            $confScore   = (float) ($est?->confidence_score ?? 0);
                        ...
                        <div class="bg-card rounded-2xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] flex flex-col overflow-hidden hover:shadow-[0_4px_12px_rgba(0,0,0,0.09)] transition-shadow">
                            ...
                            <div class="p-5 flex flex-col gap-4 flex-1">
                                ...
                                <div class="flex items-center justify-between mt-auto pt-1">
                                    ...
                                    @if($project->estimations_count > 0)
                                        <span class="font-['DM_Sans'] text-[11px] text-muted-foreground">
                                            {{ $project->estimations_count }} estimasi
                                        </span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="px-5 pb-5 flex gap-2">
                                ...
                                <form method="POST"
                                      action="{{ route('user.projects.destroy', $project) }}"
                                      onsubmit="return confirm('Hapus project \'{{ addslashes($project->name) }}\'? Semua estimasi di dalamnya juga akan dihapus.')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
