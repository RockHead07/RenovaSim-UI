...
@section('content')
<div class="space-y-4" x-data="partnersPage()">
    ...
    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
        <div class="relative flex-1">
            ...
            <input type="text" x-model="search" placeholder="Search by partner name…"
            ...
            <div class="bg-card rounded-[12px] border border-border/10 p-4 space-y-3">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                            <div class="w-9 h-9 rounded-full flex items-center justify-center text-[11px] font-semibold text-foreground shrink-0 bg-primary overflow-hidden">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-border/10">
                            ...
                            <th class="text-[10px] uppercase tracking-widest text-paragraph font-sans font-normal text-left px-5 py-3">Order</th>
                    ...
                    <tbody>
                        <template x-if="filtered().length === 0">
                            <tr><td colspan="6" class="text-center text-paragraph text-sm py-8">No partners found.</td></tr>
                        </template>
                        <template x-for="p in filtered()" :key="p.id">
                            <tr class="hover:bg-muted/50 transition-colors duration-200 border-b border-border/5">
                                ...
                                <td class="px-5 py-3 text-sm font-sans text-foreground">
                                    <div class="w-8 h-8 rounded bg-primary flex items-center justify-center text-foreground text-xs font-sans font-medium overflow-hidden">
                                        <template x-if="p.logo_image">
                                            <img :src="p.logo_image" :alt="p.name" class="w-full h-full object-cover">
                                        </template>
                                        <template x-if="!p.logo_image">
