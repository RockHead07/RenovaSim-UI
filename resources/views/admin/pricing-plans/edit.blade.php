...
<div class="bg-card rounded-xl w-full max-w-4xl border border-border/10 shadow-xl shadow-black/10 mx-auto" x-data="planForm()">
  <div class="p-5 sm:p-6 pb-0"><h2 class="font-serif text-foreground text-lg sm:text-xl">Edit Plan</h2></div>
  <form method="POST" action="/admin/pricing-plans/{{ $plan->id }}" class="p-5 sm:p-6 space-y-5 sm:space-y-6">
    @csrf
    @method('PUT')

    <div class="space-y-1.5">
      <label class="block text-xs font-sans uppercase tracking-widest text-paragraph mb-1.5">Name</label>
      <input name="name" type="text" value="{{ old('name', $plan->name) }}" placeholder="Enter plan name" class="w-full bg-background border border-border text-foreground rounded-lg px-4 py-2.5 text-sm font-sans transition-colors hover:border-primary/40 focus:outline-none focus:ring-1 focus:ring-primary" required>
    </div>

    <div class="space-y-1.5">
      <label class="block text-xs font-sans uppercase tracking-widest text-paragraph mb-1.5">Description</label>
      <textarea name="description" rows="3" placeholder="Brief description for the landing page" class="w-full bg-background border border-border text-foreground rounded-lg px-4 py-2.5 text-sm font-sans transition-colors hover:border-primary/40 focus:outline-none focus:ring-1 focus:ring-primary">{{ old('description', $plan->description) }}</textarea>
    </div>

    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div class="space-y-1.5">
        <label class="block text-xs font-sans uppercase tracking-widest text-paragraph mb-1.5">Current Price ($/mo)</label>
        <input name="price" type="number" step="0.01" min="0" value="{{ old('price', $plan->price) }}" x-model.number="finalPrice" placeholder="Enter current monthly price" class="w-full bg-background border border-border text-foreground rounded-lg px-4 py-2.5 text-sm font-sans transition-colors hover:border-primary/40 focus:outline-none focus:ring-1 focus:ring-primary" required>
      </div>
      <div class="space-y-1.5">
        <label class="block text-xs font-sans uppercase tracking-widest text-paragraph mb-1.5">Original Price (Optional)</label>
        <input name="original_price" type="number" step="0.01" min="0" value="{{ old('original_price', $plan->original_price) }}" x-model.number="originalPrice" placeholder="Example: 50" class="w-full bg-background border border-border text-foreground rounded-lg px-4 py-2.5 text-sm font-sans transition-colors hover:border-primary/40 focus:outline-none focus:ring-1 focus:ring-primary">
      </div>
    </div>
      ...
      <template x-for="(feature, index) in features" :key="index">
        <div class="tidy-panel rounded-lg p-3 grid grid-cols-1 sm:grid-cols-12 gap-2 sm:gap-3 items-center">
          ...
          <div class="sm:col-span-3 inline-flex items-center justify-between sm:justify-center gap-2">
            ...
            <div class="checkbox-wrapper-10">
              <input :id="`feature_available_edit_${index}`" :name="`features[${index}][is_available]`" type="checkbox" value="1" x-model="feature.is_available" class="tgl tgl-flip">
              <label class="tgl-btn" data-tg-off="No" data-tg-on="Yes" :for="`feature_available_edit_${index}`"></label>
            </div>
          </div>
...
@push('scripts')
@php
  $initialFeatures = old('features', $plan->features->map(function ($feature) {
    return [
      'feature' => $feature->feature,
      'is_available' => (bool) $feature->is_available,
    ];
  })->toArray());
@endphp
<script>
